...
class resource(object):
    """
    # 说明：
    用于装饰方法的注解，可以使方法成为类的同名属性，跟在init中定义Dao一样。
    使用resource的好处：
    - 变成只读属性，不会在运行中被修改
    - 懒加载，只在第一次用都的时候加载和初始化。
    - 缓存，对象只会被初始化一次。

    # 使用如下：
    ```
    class CanInitCheckTask(CheckTask):
        def __init__(self, conn, itl_db):
            self.conn = conn
            # 使用Resource注解之后不用再在此声明
            # self.riskSystemParamDao = RiskSystemParamDao(self.conn)

        @Resource
        def riskSystemParamDao(self):
            return RiskSystemParamDao(self.conn)
    ```
    """
...
class Dao(object):
    ...
    def select_page(self, where=None, order_by=None, first=1, last=10, **args):
        ...
        original_sql = "{0} {1} {2}".format(self._model.__select__, where_str,
                                            order_by_str)
    ...
    def find_page(self, order_by=None, first=1, last=10, **pks):
        ...
        return self.select_page(where=where,
                                order_by=order_by,
                                first=first,
                                last=last,
                                **args)
    ...
    def find_one_with_lock(self, nowait=False, time_out=5, **pks):
        ...
        _lock_sql = "{0} {1} {2}".format(self._model.__select__, where,
                                         for_update_sql)
